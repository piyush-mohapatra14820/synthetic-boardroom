...
export default function Dashboard() {
  ...
  async function createGroup() {
    ...
    const { data: group, error } = await supabase.from('groups').insert({
      name: newName.trim(), description: newDesc.trim(), created_by: user.id
    }).select().single();
      ...
      <div className="max-w-xl mx-auto relative z-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[#5c3010] text-sm mt-0.5" style={PLAYFAIR}>{user?.email}</p>
          </div>
          <button onClick={signOut} className="text-[#5c3010] text-xs hover:text-[#1a0e08] transition-colors border border-[#5c0403]/20 px-3 py-1.5 rounded-lg" style={PLAYFAIR}>Sign out</button>
        </div>

        {!creating ? (
          <button onClick={() => setCreating(true)}
            className="w-full mb-6 py-3.5 rounded-xl bg-[#5c0403] hover:bg-[#7a0504] text-[#f5e6d3] text-sm transition-colors shadow-md" style={OSWALD}>
            + CREATE A BOARDROOM
          </button>
        ) : (
          <div className="border border-[#5c0403]/25 bg-white/40 rounded-xl p-5 mb-6 backdrop-blur-sm space-y-3">
            <div className="text-sm text-[#1a0e08] font-medium" style={OSWALD}>NEW BOARDROOM</div>
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Name (e.g. Tagda Raho Council)" className={inputCls} />
        ...
        {loading ? (
          <div className="text-[#5c3010] text-sm" style={PLAYFAIR}>Loading your boardrooms...</div>
        ) : groups.length === 0 ? (
          <div className="text-center py-16">
            <div className="text-[#5c3010] text-sm mb-2" style={PLAYFAIR}>No boardrooms yet.</div>
            <div className="text-[#5c3010]/60 text-xs" style={PLAYFAIR}>Create one or accept an invite link from a colleague.</div>
          </div>
        ) : (
          <div className="space-y-3">
            {groups.map(g => (
              <button key={g.id} onClick={() => router.push(`/group/${g.id}`)}
                className="w-full text-left border border-[#5c0403]/20 bg-white/35 rounded-xl px-5 py-4 hover:bg-white/50 hover:border-[#5c0403]/40 transition-all group backdrop-blur-sm">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-[#5c0403]/60 text-xs mt-1" style={PLAYFAIR}>{g.session_count || 0} sessions · invite: <span className="font-mono">{g.invite_code}</span></div>
                  </div>
