    ...
    <main className="wood-grain min-h-screen px-5 py-10 relative z-10 overflow-x-hidden">
      <div className="max-w-xl mx-auto relative z-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={signOut} className="text-[#5c3010] text-xs hover:text-[#1a0e08] transition-colors border border-[#5c0403]/20 px-3 py-1.5 rounded-lg" style={PLAYFAIR}>
            Sign out
          </button>
        </div>
        ...
        {!creating ? (
          <button onClick={() => setCreating(true)}
            className="w-full mb-6 py-3.5 rounded-xl bg-[#5c0403] hover:bg-[#7a0504] text-[#f5e6d3] text-sm transition-colors shadow-md"
            style={OSWALD}>
            + CREATE A BOARDROOM
          </button>
        ) : (
              ...
              <button key={g.id} onClick={() => router.push(`/group/${g.id}`)}
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-[#5c0403]/60 text-xs mt-1" style={PLAYFAIR}>
                      {g.session_count || 0} sessions · invite: <span className="font-mono">{g.invite_code}</span>
                    </div>
                  </div>
