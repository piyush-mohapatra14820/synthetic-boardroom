...
export default function InvitePage() {
  ...
  async function acceptInvite() {
    ...
    if (err && !err.message.includes("duplicate")) {
      setError(err.message); setStatus("ready"); return;
    }
  ...
  if (status === "notfound") return (
    <main className="wood-grain min-h-screen flex items-center justify-center relative z-10">
      <div className="text-center">
        <div className="text-[#1a0e08] font-medium mb-2" style={OSWALD}>INVITE NOT FOUND</div>
        <div className="text-[#5c3010] text-sm" style={PLAYFAIR}>This invite link may be invalid.</div>
      </div>
    </main>
  );
      ...
      <div className="w-full max-w-sm relative z-10 fade-up text-center">
        ...
        {status === "needslogin" ? (
          <div>
            ...
            <button onClick={() => router.push(`/login?next=/invite/${code}`)}
              className="w-full bg-[#5c0403] text-[#f5e6d3] py-3.5 rounded-lg text-sm" style={OSWALD}>
              SIGN IN TO JOIN →
            </button>
          </div>
        ) : (
          <button onClick={acceptInvite} disabled={status === "joining"}
            className="w-full bg-[#5c0403] hover:bg-[#7a0504] text-[#f5e6d3] py-3.5 rounded-lg text-sm transition-colors disabled:opacity-40 shadow-md"
            style={OSWALD}>
